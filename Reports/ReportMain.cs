...
namespace PingCastleCloud.Reports
{
    public class ReportMain : ReportBase
    {
        public string GenerateReportFile(HealthCheckCloudData report, PingCastleLicense license, string filename)
        {
            Report = report;
            _license = license;
        ...
        PingCastleLicense _license;

        public string GenerateRawContent(HealthCheckCloudData report)
        {
            Report = report;
            sb.Length = 0;
            GenerateContent();
        ...
        protected override void ReferenceJSAndCSS()
        {
            ...
            AddScript(TemplateManager.LoadReportMainJs());
        }
...
");
        ...
        protected void GenerateAccordionDetailForRule(string id, string dataParent, string title, HealthCheckCloudDataRiskRule rule, RuleBase hcrule, GenerateContentDelegate content)
        {
    ...
    <li><span class='badge grade-5'>5</span> The Active Directory infrastructure correctly implements the latest state-of-the-art administrative model and security features.</li>
        ...
        void GenerateMitreTechnique()
        {
            ...
            foreach (MitreAttackMainTechnique mainTechnique in Enum.GetValues(typeof(MitreAttackMainTechnique)))
            {
                ...
                Add("<div class='card-body'>");
                Add("<h5 class='card-title'>");
                var description = GetEnumDescription(mainTechnique);
                Add(description);
                Add("</h5>");
                int num = 0;
