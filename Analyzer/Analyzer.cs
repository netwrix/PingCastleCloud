...
using PingCastleCloud.PublicServices;
using PingCastleCloud.RESTServices;
using PingCastleCloud.RESTServices.Azure;
using PingCastleCloud.RESTServices.O365;
using PingCastleCloud.Rules;
using System;
...
namespace PingCastleCloud.Analyzer
{
    public class Analyzer
    {
        ...
        private void AnalyzeKnownTenant()
        {
            ...
            if (p.responses != null && p.responses.Count > 0)
            {
                ...
                if (string.IsNullOrEmpty(data.TenantName))
                {
                    data.TenantName = p.responses[0].content.value
        ...
        private void ExtractNetworkPolicy(GraphAPI.PolicyResponse policy)
        {
            ...
            {
                var policyDetail = PingCastleCloud.RESTServices.Azure.GraphAPI.PolicyDetail.LoadFromString(detail);
        ...
        private void AnalyzeAllUsers(Dictionary<Guid, HealthCheckCloudDataRoleMember> adminCache)
        {
            var appPermissionsToComplete = new Dictionary<Guid, List<HealthCheckCloudDataApplicationOAuth2PermissionGrant>>();
            if (data.Applications != null)
            {
                foreach (var app in data.Applications)
                {
                    foreach (var delegation in app.DelegatedPermissions)
                    {
                        ...
                        {
                            Guid userGuid;
                            if (Guid.TryParse(delegation.principalId, out userGuid))
                            {
                                if (!appPermissionsToComplete.ContainsKey(userGuid))
                                    appPermissionsToComplete[userGuid] = new List<HealthCheckCloudDataApplicationOAuth2PermissionGrant>();
                                appPermissionsToComplete[userGuid].Add(delegation);
                            }
                        }
                    }
                }
        ...
        private Dictionary<Guid, HealthCheckCloudDataRoleMember> AnalyzeAdminRoles()
        {
            ...
            var cache = new Dictionary<Guid, HealthCheckCloudDataRoleMember>();
                    ...
                    if (role.ObjectId != null)
                    {
                        ...
                        foreach (var member in members.ReturnValue.Results)
                        {
                            ...
                            lock (cache)
                            {
                                ...
                                if (member.ObjectId != null && !cache.ContainsKey((Guid)member.ObjectId))
                                {
                                    ...
                                    cache[(Guid)member.ObjectId] = m;
                                }
                                else
                                {
                                    m = cache[(Guid)member.ObjectId];
                                }
                                r.members.Add(m);
                            }
                        }
